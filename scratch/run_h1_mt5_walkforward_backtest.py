...
print("="*85)
print("WORLDQUANT STRICT QUANT AUDIT: MT5 REAL H1 BARS WALK-FORWARD PIPELINE")
print("="*85)
...
h1_bars_count = 3000 # ~ 5 months of continuous H1 data per symbol
...
for sym in symbols_mt5:
    ...
    df_sym['kama'] = close.ewm(span=10).mean()
    ...
    # Direction signal from lagged KAMA & ADX
    df_sym['signal'] = 0
    df_sym.loc[(df_sym['kama_diff_lag1'] > 0) & (df_sym['ADX_lag1'] >= 20.0), 'signal'] = 1  # BUY
    df_sym.loc[(df_sym['kama_diff_lag1'] < 0) & (df_sym['ADX_lag1'] >= 20.0), 'signal'] = -1 # SELL
    
    # Define Target: Next bar H1 return direction
...
# -------------------------------------------------------------
# TASK 3: WALK-FORWARD 70% IS / 30% OOS SPLIT
# -------------------------------------------------------------
...
print(f"  - In-Sample (IS: Train):  {len(df_is)} H1 bars")
print(f"  - Out-of-Sample (OOS: Blind Test): {len(df_oos)} H1 bars")
...
# Apply ML Gatekeeper Threshold: P(Loss) < 0.48 for high-probability H1 setups
approved_oos_mask = (probs_loss_oos < 0.48) & (df_oos['signal'] != 0)
df_oos_approved = df_oos[approved_oos_mask].copy().reset_index(drop=True)

print(f"\n[3] OOS ML FILTER AUDIT:")
print(f"  - Total OOS H1 Bars: {len(df_oos)}")
print(f"  - Total OOS Approved Signals: {len(df_oos_approved)} ({(len(df_oos_approved)/len(df_oos))*100:.2f}%)")
...
fixed_lot = 0.01 # Standard Micro Lot
closed_trades_oos = []
...
for idx, row in df_oos_approved.iterrows():
    ...
    # Trailing BE Floor exit (15% event)
...
print("\n" + "="*85)
print("WORLDQUANT REALISTIC OUT-OF-SAMPLE (OOS 2026) AUDIT METRICS (MT5 REAL BARS)")
print("="*85)
...
report_oos_md = f"""# 🏛️ WORLDQUANT OOS WALK-FORWARD QUANT AUDIT REPORT (MT5 REAL H1 BARS)
...
for i, r in df_oos_res.head(5).iterrows():
    report_oos_md += f"* **Trade #{i+1}**: Symbol: `{r['symbol']}` | Direction: `{r['direction']}` | Entry Px: `{r['entry_price']:.5f}` | Lagged RSI_H1: `{r['lagged_rsi']:.2f}` | Lagged ATR: `{r['lagged_atr']:.5f}` | Net PnL: `${r['net_usd']:.2f}` ({r['net_pips']:.1f} pips)\n"

with open("scratch/mt5_h1_oos_audit_report.md", "w", encoding="utf-8") as f:
    f.write(report_oos_md)

print("\nAudit Report written to scratch/mt5_h1_oos_audit_report.md")
