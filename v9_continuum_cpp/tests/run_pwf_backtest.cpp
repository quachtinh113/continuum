...
int main() {
    std::cout << "==========================================================================================" << std::endl;
    std::cout << "     CONTINUUM V9 C++: 18-MONTH ROLLING PURGED WALK-FORWARD & SURVIVAL QUANT AUDIT        " << std::endl;
    std::cout << "==========================================================================================" << std::endl;
    std::cout << " Engine Core          : Modern C++ (Microsecond Execution)" << std::endl;
    std::cout << " Multi-Asset Universe : XAUUSD (Gold), USDJPY, AUDUSD, USDCAD" << std::endl;
    std::cout << " Sizing Engine        : Fixed Fractional Risk 0.5% + Micro-Account Quantization Guard" << std::endl;
    std::cout << " ML Gatekeeper        : C++ Decision Tree Ensemble (ML_VETO_THRESHOLD = 0.80)" << std::endl;
    ...
    std::vector<std::string> symbols = {"XAUUSD", "USDJPY", "AUDUSD", "USDCAD"};

    // 4 Rolling Purged Walk-Forward Folds across 26,000+ M15 bars (~18 months)
    // Fold 1: Bars 0 -> 6,500
    // Fold 2: Bars 6,500 -> 13,000
    // Fold 3: Bars 13,000 -> 19,500
    // Fold 4: Bars 19,500 -> 26,000
    struct FoldConfig {
        int fold_id;
        size_t start_bar;
        size_t num_bars;
    ...
    std::cout << "[Phase 1] Executing C++ Purged Walk-Forward Folds across Historical Dataset..." << std::endl;
    std::cout << "------------------------------------------------------------------------------------------" << std::endl;
    ...
    std::cout << "\n==========================================================================================" << std::endl;
    std::cout << "      WORLDQUANT INSTITUTIONAL PERFORMANCE & SURVIVAL MATRIX (C++ ENGINE)                 " << std::endl;
    std::cout << "==========================================================================================" << std::endl;
    ...
    print_row("Win Rate (Tỷ lệ thắng)", "45.0% - 65.0%", std::to_string(static_cast<int>(win_rate)) + "." + std::to_string(static_cast<int>(win_rate * 100) % 100) + "%", win_rate >= 45.0 && win_rate <= 70.0);
    print_row("Profit Factor (Lãi/Lỗ)", ">= 1.50", std::to_string(profit_factor).substr(0, 4), profit_factor >= 1.50);
    print_row("Net Profit (Lợi nhuận ròng)", "> $0.00", "$" + std::to_string(static_cast<int>(net_pnl)) + " (" + std::to_string(return_pct).substr(0, 5) + "%)", net_pnl > 0);
    ...
    print_row("Max Consecutive Losses", "<= 5 lệnh", std::to_string(max_cons) + " lệnh", max_cons <= 5);
    ...
    std::cout << "  • Xác suất sống sót ở Max DD <= 10% : " << std::fixed << std::setprecision(1) << mc.survival_rate_10dd << "%" << std::endl;
    ...
    std::cout << "  • Vốn kỳ vọng trung vị ($10,000)    : $" << std::fixed << std::setprecision(2) << mc.median_final_equity << std::endl;
    std::cout << "==========================================================================================" << std::endl;

    std::cout << "\nCHI TIẾT PNL THEO TỪNG TÀI SẢN (4 CORE ASSETS):" << std::endl;
    std::cout << "--------------------------------------------------" << std::endl;
